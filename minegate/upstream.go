...
func (upstream *Upstream) Validate() (valid bool) {
	...
	if err != nil {
		...
		if host, port, err = net.SplitHostPort(server); err != nil {
			log.Error("无法识别所添加的服务器: " + upstream.Server)
			return false
		}
		log.Infof("在配置文件中%s未发现特定的服务器端口, 默认使用 25565", upstream.Server)
		upstream.Server = server
	}
	p, err := strconv.ParseUint(port, 10, 16)
	if err != nil {
		log.Errorf("错误的端口 %s: %s", port, err.Error())
		return false
	}
	host = strings.ToLower(host)
	if !CheckHost(host) {
		log.Error("错误的主机名（域名）: " + host)
		return false
	}
	...
	if !CheckPattern(upstream.Pattern) {
		log.Error("错误的代理模式: " + upstream.Pattern)
		return false
	}
	if upstream.ErrorMsg.Text == "" {
		log.Warnf("为 %s 清空错误信息, 重载日志", upstream.Server)
		upstream.ErrorMsg.Text = "无法连接至服务器：" + upstream.Server
	}
